...
async def create_group(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid member IDs format")
    ...
    users_count = db.query(func.count(User.id)).filter(User.id.in_(member_id_list)).scalar()
    if users_count != len(member_id_list):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="One or more members don't exist")
...
async def add_group_members(
    ...
    if users_count != len(members):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="One or more users don't exist")
    
    # Get existing members to avoid duplicates
    existing_members = db.query(room_members.c.user_id).filter(
        room_members.c.room_id == room_id
    ).all()
...
async def remove_group_member(
    ...
    if is_admin and current_user.id != user_id:
        target_is_admin = db.query(room_members).filter(
            and_(
                room_members.c.room_id == room_id,
                room_members.c.user_id == user_id,
                room_members.c.is_admin == True
        ...
        if target_is_admin:
            # Count other admins
            other_admins_count = db.query(func.count(room_members.c.user_id)).filter(
                and_(
                    room_members.c.room_id == room_id,
                    room_members.c.user_id != user_id,
                    room_members.c.is_admin == True
    ...
    members_count = db.query(func.count(room_members.c.user_id)).filter(
        room_members.c.room_id == room_id
    ).scalar()
    ...
    if members_count == 0:
        ...
        return {"status": "success", "message": "Member removed and empty group deleted"}
...
async def leave_group(
    ...
    if is_admin:
        ...
        if other_admins_count == 0:
            # Count total members
            total_members = db.query(func.count(room_members.c.user_id)).filter(
                room_members.c.room_id == room_id
            ).scalar()
            
            if total_members > 1:
                # Promote someone else to admin if there are other members
                next_admin = db.query(room_members).filter(
                    and_(
                        room_members.c.room_id == room_id,
                        room_members.c.user_id != current_user.id
                    )
...
async def make_group_admin(
    ...
    is_already_admin = db.query(room_members).filter(
        and_(
            room_members.c.room_id == room_id,
            room_members.c.user_id == user_id,
            room_members.c.is_admin == True
